...
uint256 hashGenesisBlock("0x95793c244e559ea3da5f7058ecd678522121298442935e74838684d30c04ebed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // XAGz: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "XAGz Signed Message:\n";
...
{
    // XAGz: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // XAGz: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // XAGz: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // 1 Oz of XAGz: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // XAGz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // XAGz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me XAGz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x0fd4c0dc7c6391b2f4019ef4c3a9f6c5f69ad47f444df41dfb780052ba98d5ec7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // XAGz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XAGzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XAGzMiner : ProcessBlock, block not accepted");
    }
